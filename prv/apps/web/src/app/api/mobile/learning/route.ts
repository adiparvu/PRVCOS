...
export const GET = withMobileAuth(async (req: NextRequest, ctx) => {
  ...
  const statusFilter = sp.get("status") as CourseStatus | null
  const categoryFilter = sp.get("category") as CourseCategory | null
  ...
  const [courseRows, enrollmentRows, achievementRows] = await Promise.all([
    ...
    db
      ...
      .where(and(eq(courseEnrollments.companyId, companyId), eq(courseEnrollments.userId, userId))),
  ...
  const achievements = achievementRows.map((a) => ({
    ...
    label: a.label,
    detail: a.detail ?? "",
    date: `${a.achievedAt.getDate()} ${MONTH_LABELS[a.achievedAt.getMonth()]}`,
  ...
  const active = enrollmentRows.filter((e) => e.status === "in_progress" || e.status === "completed")
  const avgScore =
    active.length > 0
