...
export function LeadBuilderClient() {
  ...
  const [form, setForm] = useState<FormState>({ name: "", company: "", email: "", phone: "", source: "website", estimatedValue: "", notes: "" })
      ...
      <div style={{ minHeight: "100vh", background: "#000", display: "flex", alignItems: "center", justifyContent: "center", padding: 24 }}>
        <div style={{ ...GLASS_CARD, padding: 40, textAlign: "center", maxWidth: 400 }}>
          ...
          <div style={{ display: "flex", gap: 12, justifyContent: "center" }}>
            <button style={BTN_PRIMARY} onClick={() => router.push("/crm/leads")}>← Înapoi la listă</button>
            <button style={BTN_GHOST} onClick={() => { setDone(false); setForm({ name: "", company: "", email: "", phone: "", source: "website", estimatedValue: "", notes: "" }) }}>+ Creează altul</button>
          </div>
        </div>
      </div>
    )
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#000", padding: "24px 16px 120px" }}>
      <div style={{ maxWidth: 600, margin: "0 auto" }}>
        ...
        <h1 style={{ color: "rgba(255,255,255,0.95)", fontSize: 28, fontWeight: 700, marginBottom: 32 }}>Lead Nou</h1>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div>
            <label style={LABEL}>Companie</label>
            <input style={FIELD} value={form.company as string} onChange={e => set(e, "company")} type="text" />
          </div>
          <div>
            <label style={LABEL}>Email *</label>
            <input style={FIELD} value={form.email as string} onChange={e => set(e, "email")} type="email" required />
          </div>
          <div>
            <label style={LABEL}>Telefon *</label>
            <input style={FIELD} value={form.phone as string} onChange={e => set(e, "phone")} type="text" required />
          </div>
          <div>
            <label style={LABEL}>Sursă *</label>
            <select style={SELECT} value={form.source as string} onChange={e => set(e, "source")}>
              <option value="">— Selectează —</option>
              <option value="website">Site web</option>
              <option value="referral">Recomandare</option>
              <option value="cold_call">Apel la rece</option>
          ...
          <div>
            <label style={LABEL}>Valoare estimată (RON)</label>
            <input style={FIELD} value={form.estimatedValue as string} onChange={e => set(e, "estimatedValue")} type="number" step="1" />
          </div>
          <div>
