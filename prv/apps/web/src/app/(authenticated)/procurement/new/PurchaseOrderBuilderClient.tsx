...
export function PurchaseOrderBuilderClient() {
  ...
  const [form, setForm] = useState<FormState>({ ref: "", description: "", supplierName: "", amount: "", date: "", neededBy: "", notes: "" })
      ...
      <div style={{ minHeight: "100vh", background: "#000", display: "flex", alignItems: "center", justifyContent: "center", padding: 24 }}>
        <div style={{ ...GLASS_CARD, padding: 40, textAlign: "center", maxWidth: 400 }}>
          ...
          <div style={{ display: "flex", gap: 12, justifyContent: "center" }}>
            <button style={BTN_PRIMARY} onClick={() => router.push("/procurement")}>← Înapoi la listă</button>
            <button style={BTN_GHOST} onClick={() => { setDone(false); setForm({ ref: "", description: "", supplierName: "", amount: "", date: "", neededBy: "", notes: "" }) }}>+ Creează altul</button>
          </div>
        </div>
      </div>
    )
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#000", padding: "24px 16px 120px" }}>
      <div style={{ maxWidth: 600, margin: "0 auto" }}>
        ...
        <h1 style={{ color: "rgba(255,255,255,0.95)", fontSize: 28, fontWeight: 700, marginBottom: 32 }}>Comandă de Achiziție Nouă</h1>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div>
            ...
            <input style={FIELD} value={form.ref as string} onChange={e => set(e, "ref")} type="text" placeholder="PO-001" required />
          </div>
          <div>
          ...
          <div>
            ...
            <input style={FIELD} value={form.supplierName as string} onChange={e => set(e, "supplierName")} type="text" />
          </div>
          <div>
            <label style={LABEL}>Valoare (RON) *</label>
            <input style={FIELD} value={form.amount as string} onChange={e => set(e, "amount")} type="number" step="1" />
          </div>
          <div>
            <label style={LABEL}>Data comenzii *</label>
            <input style={FIELD} value={form.date as string} onChange={e => set(e, "date")} type="date" />
          </div>
          <div>
            <label style={LABEL}>Necesar până la</label>
            <input style={FIELD} value={form.neededBy as string} onChange={e => set(e, "neededBy")} type="date" />
          </div>
          <div>
