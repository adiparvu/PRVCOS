...
export function AttendanceWorkspace() {
  ...
  const employees = useMemo<Employee[]>(() => (data?.records ?? []).map(mapRecord), [data?.records])
    ...
    return (
      ...
      >
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 20 }}>
          ...
          <div>
            <div style={{ fontSize: 18, fontWeight: 700, color: "var(--prv-text-1)" }}>
              {emp.name}
            </div>
            <div style={{ fontSize: 13, color: t3, marginTop: 2 }}>
              {emp.role} · {emp.location} · {dateLabel}
            </div>
          </div>
        ...
        >
          {[
            ...
            >
              <div style={{ fontSize: 18, fontWeight: 700, color: s.color ?? "var(--prv-text-1)" }}>
                {s.val}
              </div>
              <div
                style={{
          ...
          {[
            ...
            emp.clockOut
              ? {
                  time: emp.clockOut,
                  label: "Clock Out",
                  sub: emp.duration ? `Active ${emp.duration}` : "Shift ended",
                  color: t3,
                }
              : null,
            !emp.clockIn && emp.status === "Absent"
              ? { time: "—", label: "No check-in", sub: "Employee absent today", color: red }
              : null,
            !emp.clockIn && emp.status === "On Leave"
              ? { time: "—", label: "On Leave", sub: emp.leaveRange ?? "Leave period", color: blue }
              : null,
          ]
            .filter(Boolean)
            .map((ev, i, arr) => (
              ...
              >
                ...
                />
                <div style={{ width: 46, flexShrink: 0, fontSize: 12, color: t3, marginTop: 1 }}>
                  {ev!.time}
                </div>
                <div>
                  <div
                    style={{
                      fontSize: 13,
                      fontWeight: 600,
                      color: ev!.time === "—" ? t3 : "var(--prv-text-1)",
                    }}
                  >
                    {ev!.label}
                  </div>
              ...
              <>
                ...
                >
                  {days.map((d) => (
                    ...
                    >
                      <div style={{ fontSize: 10, color: d.isToday ? t3 : green, fontWeight: 700 }}>
                        {d.isToday ? "now" : "8h"}
                      </div>
                      <div
                        style={{
                          width: "100%",
                          height: d.isToday ? 22 : 30,
                          borderRadius: 4,
                          background: d.isToday ? "rgba(48,209,88,0.25)" : "rgba(48,209,88,0.45)",
                        }}
                      />
                    </div>
                  ))}
                  ...
                  {days.map((d) => (
                    <div
                      ...
                      style={{
                        flex: 1,
                        fontSize: 9,
                        color: d.isToday ? "var(--prv-text-1)" : t3,
                        textAlign: "center",
  ...
  return (
    ...
    >
        ...
        {[
          {
            val: isLoading
              ...
              : String(
                  meta?.present ??
                    employees.filter((e) => e.status === "Present" || e.status === "Clocked Out")
                      .length
                ),
            label: "Present",
            color: green,
          },
          {
            val: isLoading
              ? "…"
              : String(meta?.late ?? employees.filter((e) => e.status === "Late").length),
            label: "Late",
            color: amber,
          },
          {
            val: isLoading
              ? "…"
              : String(meta?.absent ?? employees.filter((e) => e.status === "Absent").length),
            label: "Absent",
            color: red,
          },
          {
            val: isLoading
              ? "…"
              : String(meta?.onLeave ?? employees.filter((e) => e.status === "On Leave").length),
            label: "On Leave",
            color: blue,
          },
        ].map((k) => (
      ...
      >
        ...
        <button
          ...
          style={{
            ...
            border: "none",
            cursor: "pointer",
            background: clockedIn ? "rgba(255,69,58,0.80)" : "rgba(48,209,88,0.85)",
            color: clockedIn ? "#fff" : "#000",
            transition: "all 0.2s",
          }}
        >
        ...
        {[
          ...
          <div
            ...
            style={{
              ...
              padding: "8px 4px",
              borderRadius: 12,
              background: d.today ? g2 : "transparent",
              border: d.today ? `1px solid var(--prv-g2)` : "1px solid transparent",
            }}
          >
            <div
              style={{ fontSize: 10, fontWeight: 600, color: d.today ? "var(--prv-text-1)" : t3 }}
            >
              {d.name}
            </div>
            <div
              style={{
                fontSize: 15,
                fontWeight: 700,
                color: d.today ? "var(--prv-text-1)" : "var(--prv-text-1)",
              }}
            >
              {d.num}
            </div>
      ...
      <div style={card}>
        <TopEdge />
        {isLoading ? (
          <div style={{ padding: "32px 16px", textAlign: "center", color: t3, fontSize: 14 }}>
            Loading attendance…
          </div>
        ) : filtered.length === 0 ? (
          <div style={{ padding: "32px 16px", textAlign: "center", color: t3, fontSize: 14 }}>
