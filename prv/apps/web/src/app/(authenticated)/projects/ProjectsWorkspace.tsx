...
const PROJECTS: Project[] = [
  {
    ...
    team: [
      { initials: "AP", name: "Andrei Popescu", role: "Project Manager",  status: "online",  statusLabel: "On Site"   },
      { initials: "MR", name: "Marius Roșu",   role: "Site Supervisor",   status: "online",  statusLabel: "On Site"   },
      { initials: "LT", name: "Liviu Toma",     role: "Tile Specialist",   status: "online",  statusLabel: "On Site"   },
      { initials: "DP", name: "Dan Pop",        role: "Electrician",       status: "away",    statusLabel: "En Route"  },
      { initials: "IC", name: "Ion Crișan",     role: "Plumber",           status: "offline", statusLabel: "Off Shift" },
    ],
  ...
  },
  {
    ...
    team: [
      { initials: "EM", name: "Elena Marin",  role: "Project Manager", status: "online",  statusLabel: "On Site"   },
      { initials: "RC", name: "Relu Ciobanu", role: "Site Supervisor", status: "online",  statusLabel: "On Site"   },
      { initials: "GS", name: "Gelu Stan",    role: "Plumber",         status: "away",    statusLabel: "En Route"  },
      { initials: "IA", name: "Ion Avram",    role: "Carpenter",       status: "offline", statusLabel: "Off Shift" },
    ],
  ...
  },
  {
    ...
    team: [
      { initials: "RD", name: "Radu Dima",    role: "Project Manager", status: "online",  statusLabel: "On Site" },
      { initials: "IN", name: "Iosif Nistor", role: "Tile Specialist", status: "online",  statusLabel: "On Site" },
      { initials: "BM", name: "Bogdan Mol.",  role: "Plumber",         status: "offline", statusLabel: "Done"    },
    ],
  ...
  },
  {
    ...
    team: [
      { initials: "VN", name: "Vlad Nicu",   role: "Project Manager",  status: "online",  statusLabel: "On Site" },
      { initials: "TM", name: "Titu Moga",   role: "Site Supervisor",  status: "online",  statusLabel: "On Site" },
      { initials: "SF", name: "Sorin Florea",role: "Structural Eng.",  status: "away",    statusLabel: "En Route"},
    ],
  ...
  },
  {
    ...
    team: [
      { initials: "MI", name: "Maria Ionescu", role: "Project Manager",  status: "online",  statusLabel: "Office"  },
      { initials: "DD", name: "Dorel Dănilă",  role: "Floor Specialist", status: "offline", statusLabel: "Done"    },
    ],
  ...
  return (
    ...
    >
      ...
      <div className="flex gap-3 mb-2.5 ml-5">
        <span className="text-[12px] text-white/35">PM: <span className="text-white/65 font-semibold">{project.pm}</span></span>
        <span className="text-[12px] text-white/35">Value: <span className="text-white/65 font-semibold">{project.value}</span></span>
        <span className="text-[12px] text-white/35">Due: <span className="text-white/65 font-semibold">{project.due}</span></span>
      </div>

      <div className="ml-5 mb-2">
        <GlassProgressBar value={project.pct} size="sm" color={PROGRESS_COLOR[project.status]} animated />
      </div>
        ...
        <div className="flex">
          ...
          {project.team.length > 4 && (
            <div
              ...
              style={{ background: "var(--prv-g2)", border: "1.5px solid #000", marginLeft: -6, position: "relative" }}
            >
              +{project.team.length - 4}
            </div>
          )}
    ...
    <div>
      ...
      >
        ...
        <div className="flex gap-2 flex-wrap mb-3.5">
          ...
          <span className="text-[11px] font-semibold px-2.5 py-1 rounded-[8px]" style={{ background: "var(--prv-g2)", color: "var(--prv-text-3)" }}>Renovation</span>
          <span className="text-[11px] font-semibold px-2.5 py-1 rounded-[8px]" style={{ background: "var(--prv-g2)", color: "var(--prv-text-3)" }}>{project.location}</span>
        </div>
        <GlassProgressBar value={project.pct} size="md" color={PROGRESS_COLOR[project.status]} showValue animated className="mb-3.5" />
        ...
        <GlassCard>
          <div className="p-4">
            <p className="text-[13px] text-white/65 leading-relaxed mb-3">
              Project managed by <span className="text-white/90 font-semibold">{project.pm}</span> — {project.team.length} team members assigned. Currently at <span className="text-white/90 font-semibold">{project.pct}% completion</span> with deadline on <span className="text-white/90 font-semibold">{project.due}</span>.
            </p>
            <div className="flex gap-3">
              <div className="flex-1 p-3 rounded-[12px]" style={{ background: "var(--prv-g2)" }}>
                <p className="text-[10px] font-semibold text-white/35 uppercase tracking-widest mb-1">Phases Done</p>
                <p className="text-[18px] font-bold text-white/90">{project.phases.filter(p => p.state === "done").length}/{project.phases.length}</p>
              </div>
              <div className="flex-1 p-3 rounded-[12px]" style={{ background: "var(--prv-g2)" }}>
                <p className="text-[10px] font-semibold text-white/35 uppercase tracking-widest mb-1">Team Online</p>
              ...
              <div className="flex-1 p-3 rounded-[12px]" style={{ background: "var(--prv-g2)" }}>
                <p className="text-[10px] font-semibold text-white/35 uppercase tracking-widest mb-1">Budget Used</p>
      ...
      {tab === "phases" && (
        <GlassCard>
          {project.phases.map((phase) => {
            ...
            return (
              <div key={phase.num} className="flex items-center gap-3 px-4 py-3.5" style={{ borderBottom: "1px solid var(--prv-border-subtle)" }}>
                <div
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {phase.state === "active" && (
                    <div className="mt-2">
                      <GlassProgressBar value={phase.pct} size="sm" color={PHASE_PROGRESS_COLOR[phase.state]} animated />
                    </div>
                  )}
                </div>
      ...
      {tab === "team" && (
        <GlassCard>
          {project.team.map((member) => (
            <div key={member.initials} className="flex items-center gap-3 px-4 py-3" style={{ borderBottom: "1px solid var(--prv-border-subtle)" }}>
              <div
              ...
              <div className="flex items-center gap-1.5">
                <GlassStatusDot kind={member.status} size="sm" />
                <span className="text-[11px] font-semibold text-white/45">{member.statusLabel}</span>
              </div>
            </div>
          ))}
        </GlassCard>
      ...
      {tab === "financials" && (
        <GlassCard>
          {[
            { label: "Estimated",    value: project.financials.estimated, color: "white"  as const, pct: 100 },
            { label: "Actual Spent", value: project.financials.spent,     color: "blue"   as const, pct: Math.round((project.financials.spent    / project.financials.estimated) * 100) },
            { label: "Invoiced",     value: project.financials.invoiced,  color: "green"  as const, pct: Math.round((project.financials.invoiced  / project.financials.estimated) * 100) },
            { label: "Remaining",    value: remaining > 0 ? remaining : 0,color: "orange" as const, pct: Math.round((Math.max(remaining, 0)       / project.financials.estimated) * 100) },
          ].map(({ label, value, color, pct }) => (
            <div key={label} className="flex items-center gap-3 px-4 py-3.5" style={{ borderBottom: "1px solid var(--prv-border-subtle)" }}>
    ...
    <div className="px-4 pt-14 pb-28 max-w-2xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-5">
        <div>
          ...
          <h1 className="text-white/90 text-[26px] font-semibold tracking-tight leading-tight">Projects</h1>
        </div>
      ...
      {/* Project list */}
      <div className="rounded-[18px] overflow-hidden relative" style={{ background: "var(--prv-g1)", border: "1px solid var(--prv-border-subtle)" }}>
        {filtered.length === 0 ? (
